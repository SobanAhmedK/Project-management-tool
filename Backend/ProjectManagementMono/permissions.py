...
#     """
#     Custom permission to allow actions on Comment only if the user:
#     - For create: is a project member or org admin/manager.
#     - For update/delete: is a project member and either the owner or, if not, an org admin/manager.
#     - For read: Only comments of tasks in projects where the user is a member or an org admin/manager.
#     """

#     def has_permission(self, request, view):
#         print("Checking general permission for comment...")

#         if request.method in permissions.SAFE_METHODS:
#             return True

#         # ✅ Only check this for creating comments
...
#             user = request.user
#             is_project_member = task.project.members.filter(id=user.id).exists()
...
#             return is_project_member or is_org_admin_manager

#         # ✅ For all other methods (PUT, PATCH, DELETE), let has_object_permission handle it
#         return True


#     def has_object_permission(self, request, view, obj):
#         print("Checking object permission for comment...")
#         user = request.user

#         if request.method in permissions.SAFE_METHODS:
#             # Allow if user is a project member or org admin/manager
#             is_project_member = obj.task.project.members.filter(id=user.id).exists()
#             is_org_admin_manager = OrganizationMembership.objects.filter(
...
#         # Allow owner
#         if obj.commented_by == user:
#             return True

#         # Also allow project admins/managers
#         is_project_admin_or_manager = OrganizationMembership.objects.filter(
#             organization=obj.task.project.organization,
#             user=user,
#             role__in=['admin', 'manager']
#         ).exists()

#         return is_project_admin_or_manager
